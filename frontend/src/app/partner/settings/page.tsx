  ...
  return (
    <div className="bg-background text-on-background font-body min-h-screen pb-20 md:pb-0">
      ...
      <main className="max-w-2xl mx-auto px-6 py-8 space-y-8">
        <h1 className="font-headline text-3xl font-extrabold text-primary mb-2">Cài đặt đối tác / パートナー設定</h1>
        <div className="bg-surface-container-lowest p-6 rounded-2xl shadow-sm flex items-center gap-6">
          ...
          <div><h2 className="font-headline font-bold text-primary text-xl">Nguyễn Thị Lan</h2><p className="text-sm text-on-surface-variant">lan.nguyen@email.com</p><p className="text-xs text-secondary mt-1">Đối tác • Ba Đình, Hà Nội</p></div>
        </div>
        <div className="space-y-2">
          {[
            {icon:"person",label:"Chỉnh sửa hồ sơ / プロフィール編集",href:"#"},
            {icon:"calendar_month",label:"Lịch trình dạy / スケジュール管理",href:"#"},
            {icon:"payments",label:"Thu nhập & Thanh toán / 収入と支払い",href:"#"},
            {icon:"notifications",label:"Thông báo / 通知設定",href:"#"},
            {icon:"lock",label:"Đổi mật khẩu / パスワード変更",href:"/change-password"},
            {icon:"help",label:"Trung tâm hỗ trợ / ヘルプセンター",href:"#"},
          ].map(item=>(
        ...
        <button onClick={()=>router.push("/login")} className="w-full flex items-center justify-center gap-2 p-4 text-error font-bold hover:bg-error-container rounded-xl transition-all">
          <span className="material-symbols-outlined">logout</span>Đăng xuất / ログアウト
        </button>
      </main>
      <PartnerBottomNav />
    </div>
